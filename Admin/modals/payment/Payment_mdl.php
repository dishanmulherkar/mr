            ...
            if ($action_status === 'approved') {
                ...
                if ($unpaid_bills->num_rows > 0) {
                    ...
                    while ($bill = $unpaid_bills->fetch_assoc()) {
                        ...
                        $grand_total = (float)$bill['grand_total']; // Original Immutable Total
                        ...
                        if ($current_cd_percent == 4) {
                            if ($bill_age_days > $cd_4_days && $bill_age_days <= $cd_2_days) {
                                // Target total if 2% was given instead of 4%
                                ...
                                // Target total if NO CD was given
                        ...
                        if ($applied_penalty > 0) {
                            // Deduct the penalty from the cash pool so it doesn't spill over to the next invoice
                        ...
                        if ($current_cd_percent == 0 && $applied_penalty == 0) {
                            ...
                            // Check if remaining cash can clear the bill AFTER discount is theoretically applied
                        ...
                        // Required Cash = (Original Total + Penalties) - (Already Paid Cash + Earned CD)
                        ...
                        // Bill is fully paid if (Cash Paid + Earned CD) >= (Original Total + Penalties)
                        ...
                        $upd_penalty = $applied_penalty; // Adds to cd_penalty_amt tracking
                        $upd_earned = $applied_earned_cd; // Adds to cd_earned_amt tracking
  ...
  public function getStockistOutstandingWithCD($stockist_id, $settlement_date = null)
{
     ...
     * Uses $safe_date instead of CURDATE() for DATEDIFF calculations
     */
    $stmt = $this->con->prepare("
        SELECT 
            inward_id,
            ...
            DATEDIFF('$safe_date', inward_date) AS bill_age_days,

            -- ALREADY GIVEN 4% CD (Target Grand Total without CD - Current Grand Total)
            CASE 
            ...
            -- ALREADY GIVEN 2% CD (Target Grand Total without CD - Current Grand Total)
            CASE 
            ...
            CASE 
                WHEN DATEDIFF('$safe_date', inward_date) <= ? AND COALESCE(cd_percent, 0) = 0 
            ...
            CASE 
                WHEN DATEDIFF('$safe_date', inward_date) > ? AND DATEDIFF('$safe_date', inward_date) <= ? AND COALESCE(cd_percent, 0) = 0 
            ...
            CASE
                WHEN COALESCE(cd_percent, 0) = 4 THEN
                    CASE 
                        WHEN DATEDIFF('$safe_date', inward_date) <= ? THEN 0 
                        
                        -- Downgrade 4% to 2% (Scale base by 98/96)
                        WHEN DATEDIFF('$safe_date', inward_date) <= ? THEN 
    ...
    // Bind the 7 parameters required for the CASE statements
    $stmt->bind_param(
        "iiiiiii",
