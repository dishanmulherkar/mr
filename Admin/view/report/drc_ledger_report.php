                        ...
                        while($row = mysqli_fetch_assoc($query)) {
                            ...
                            if ($action === 'increase') {
                                ...
                                if ($row['transaction_type'] === 'settled_to_bill' || !empty($row['settled_bill_no'])) {
                                    $bill_badge = !empty($row['settled_bill_no']) ? " <span class='badge bg-info text-dark ms-1'>" . htmlspecialchars($row['settled_bill_no']) . "</span>" : "";
                                    
                                    // FIX: Actually output the badge next to the stockist name!
                                    $particulars = $stockist_name . $bill_badge; 
                                    $vch_type = "Adjustment";
                                } else {
