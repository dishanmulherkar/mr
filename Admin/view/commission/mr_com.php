...
<div id="container">
    ...
    <div class="commission-container">
        ...
        <table class="table table-bordered">
            ...
            <tbody id="commissionTableBody">
                <?php if ($isEditMode && !empty($editData['bills'])): ?>
                    <!-- RENDER BILLS VIA PHP IN EDIT MODE -->
        ...
        <div id="adjustmentsContainer" class="mt-4 mb-5" style="padding-bottom: 80px;">
            <h5 class="mb-3 text-primary">Extra Adjustments</h5>
            <div id="adjustmentsList">
                <?php if ($isEditMode && !empty($editData['adjustments'])): ?>
                    <!-- RENDER ADJUSTMENTS VIA PHP IN EDIT MODE -->
        ...
        <div class="commission-footer" style="position: sticky; bottom: 0; background: #f8f9fa; padding: 15px; border-top: 2px solid #ddd; display: flex; justify-content: space-between; align-items: center; z-index: 100;">
            ...
            <button id="btnSubmitCommission" class="btn <?= $isEditMode ? 'btn-warning text-dark fw-bold' : 'btn-primary' ?>" disabled>
                <?= $isEditMode ? 'Save Changes' : 'Mark as Paid' ?>
            </button>
        </div>
    </div>
</div>
...
$(document).ready(function() {
    ...
    // If Edit Mode, calculate totals immediately using pre-loaded PHP data
    if (isEditMode) {
        calculateTotal();
    } else {
        // Init filters for Create Mode
        ...
        $('#state_id').change(function() {
            $.post('<?= BASE_URL ?>headquarter/getHqByStateAjax', { state_id: $(this).val() }, function(res) {
                $('#hq').html(res).trigger('change');
            });
        });

        $('#btnFetchBills').click(function() {
            let hqId = $('#hq').val();
            if (!hqId) return alert('Please select a Head Quarter first.');
    ...
    function appendEmptyAdjustmentRow() {
        let btn = isEditMode ? '<button type="button" class="btn btn-danger btn-remove-adj"><i class="fa fa-trash"></i></button>' 
                             : '<button type="button" class="btn btn-success btn-add-adj"><i class="fa fa-plus"></i></button>';
        $('#adjustmentsList').append(`
            <div class="row mb-2 adjustment-row">
                <div class="col-md-4"><input type="text" class="form-control adj-desc" placeholder="Reason"></div>
                <div class="col-md-2"><select class="form-control adj-type"><option value="-">- (Deduct)</option><option value="+">+ (Add)</option></select></div>
                <div class="col-md-3"><input type="number" class="form-control adj-amt" placeholder="Amount" step="0.01" min="0"></div>
    ...
    function calculateTotal() {
        ...
        $('#btnSubmitCommission').prop('disabled', count === 0);
    }
    
    function resetMath() { $('#totalCommission').text('0.00'); $('#selectedCount').text('0'); $('#selectAll').prop('checked', false); $('#btnSubmitCommission').prop('disabled', true); }

    // Form Submission
    $('#btnSubmitCommission').click(function() {
        let selectedBillIds = [];
        ...
        if (!confirm(`${isEditMode ? 'Save changes' : 'Mark as Paid'}? Final payout: ₹${finalTotal}`)) return;

        let btn = $(this), originalText = btn.text();
        ...
        }).fail(function() { alert('Server error occurred.'); btn.prop('disabled', false).text(originalText); });
    });
});
</script>